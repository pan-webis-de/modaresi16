...
if not os.environ.get('DATASETS', None):
    raise ValueError(
        'You need to set an environment variable DATASETS, containing the path to corpora folder.')
...
def fetch_data(remote_path=DATASETS_REMOTE):
    file_name = 'pan14-author-profiling-training-corpus-2014-04-16.zip'
    ...
    extraction_folder = os.path.join(dst_directory, 'pan14-author-profiling-training-corpus-2014-04-16')
...
def parse_xml(filename, fetch_tweet=False):
    ...
    with open(filename) as fp:
        ...
        for d in author.documents.find_all('document'):
            ...
            doc = dict(text=text)
            doc.update(d.attrs)
            if not text and fetch_tweet:
                url = d.attrs.get('url', u'')
                if url.startswith(u'https://twitter.com'):
                    text = fetch_tweet(url)
            docs.append(doc)
...
def fetch_tweet(url):
    ...
    text = soup.find_all('p', {'class': 'tweet-text'})[0].get_text()
    return text
...
def iterdocs(language,
             sections,
             basedir,
             data_path='mnt/nfs/tira/data/pan14-training-corpora-truth',
             subfolder_template='pan14-author-profiling-training-corpus-{language}-{section}-2014-04-16'):
    """Yields all documents of a given language that belong to a specified list of sections."""
    ...
    for section in sections:
        ...
        print('Loading (language={}, section={})'.format(language, section))
        ...
        for i in range(len(df)):
            ...
            author_attrs, docs = parse_xml(os.path.join(directory, labels['author_id'] + '.xml'))
...
def save(docs, language, version=VERSION):
    ...
    name = 'datasets_pan14_author_profiling-{}-{}.pgz'.format(language, version)
...
if __name__ == '__main__':
    convert()
    assert 12359 == len(load('english'))
    assert 1538 == len(load('spanish'))
